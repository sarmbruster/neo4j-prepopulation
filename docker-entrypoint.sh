...
if [ "$1" == "neo4j" ]; then
    ...
    ### this snippets has been added to original docker-entrypoint.sh
    if [ ! -z "${NEO4J_INITIAL_CYPHER_URL:=}" ]; then
        # download remote file and strip BOM if exists
        curl -L "${NEO4J_INITIAL_CYPHER_URL}" | awk 'NR==1{sub(/^\xef\xbb\xbf/,"")}{print}' > /var/lib/neo4j/import/initial.cypher
    fi

    if [ -e /var/lib/neo4j/import/initial.cypher ]; then
        echo "Starting initial data population"
        bin/neo4j start
        while ! echo exit | (nc localhost 7474 > /dev/null) ; do sleep 1; done
        if [ "${NEO4J_AUTH:-}" == "none" ]; then
            cypher_shell_auth=""
        else
            cypher_shell_auth="-u neo4j -p ${password} "

        fi
        bin/cypher-shell --non-interactive ${cypher_shell_auth} < /var/lib/neo4j/import/initial.cypher
        bin/neo4j stop
        echo "Done initial data population"
    fi
    ### end of snippet
